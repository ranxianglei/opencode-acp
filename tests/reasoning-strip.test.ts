...
function assistantMsg(id: string, modelID?: string, providerID?: string, parts?: any[]): WithParts {
    const info: any = {
        id,
        role: "assistant",
        sessionID: SID,
...
test("stripStaleMetadata removes metadata from assistant parts with different model", () => {
    ...
    stripStaleMetadata(messages)
    assert.ok(
        !("metadata" in messages[1]!.parts[0]!),
        "metadata should be stripped from different-model assistant",
    )
})
...
test("stripStaleMetadata preserves metadata for same-model assistant parts", () => {
    ...
    assert.ok(
        "metadata" in messages[1]!.parts[0]!,
        "metadata should remain for same-model assistant",
    )
})
...
test("stripStaleMetadata only strips from text/tool/reasoning parts", () => {
    ...
    assert.ok(
        "metadata" in messages[1]!.parts[3]!,
        "image metadata preserved (not text/tool/reasoning)",
    )
})
...
test("stripStaleMetadata handles undefined modelID/providerID (Bug 8 fix)", () => {
    const messages: WithParts[] = [userMsg("u1", "claude-4", "anthropic"), assistantMsg("a1")]
    stripStaleMetadata(messages)
    assert.ok(
        !("metadata" in messages[1]!.parts[0]!),
        "metadata stripped when assistant has no model info",
    )
})
...
test("stripStaleMetadata only considers the last user message's model", () => {
    ...
    assert.ok(
        !("metadata" in messages[1]!.parts[0]!),
        "a1 metadata stripped (u2 has different model)",
    )
})
