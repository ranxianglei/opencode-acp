...
test("buildContextUsageGuidance returns context number without compression guidance", () => {
    ...
    assert.match(low, /Context usage:/)
    assert.match(mid, /Context usage:/)
    assert.match(high, /Context usage:/)

    assert.doesNotMatch(low, /Be frugal|compress/i)
    assert.doesNotMatch(mid, /growing|compress/i)
    assert.doesNotMatch(high, /aggressive|compress/i)
})
