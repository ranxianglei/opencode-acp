...
function getActiveCounts(state: SessionState) {
    ...
    for (const id of state.prune.messages.activeBlockIds) {
        ...
        if (tier === 1) {
            t1++
            t1Tok += b.summaryTokens
        } else if (tier === 2) {
            t2++
            t2Tok += b.summaryTokens
        } else t3++
    }
...
test("SIM 1: 30-turn session — T1 fires, blocks accumulate, T2 escalates", async () => {
    ...
    try {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            conversation.push(
                makeAssistantMessage(`seed-a${i}`, "y".repeat(3000), 15000 + i * 5000),
            )
        }
        ...
        for (let turn = 0; turn < 30; turn++) {
            ...
            if (suffix.includes("[Tier 2 Trigger]")) {
                ...
                const t1Sum = consumed.reduce(
                    (s, id) => s + (state.prune.messages.blocksById.get(id)?.summaryTokens ?? 0),
                    0,
                )
                ...
                state.prune.messages.blocksById.set(
                    t2Id,
                    makeBlock(t2Id, 2, t2Tok, t1Sum, `T2`, ANCHOR, consumed),
                )
                ...
                state.prune.messages.blocksById.set(
                    t1Id,
                    makeBlock(t1Id, 1, t1Tok, compressed, `T1-${turn}`, ANCHOR),
                )
            ...
            events.push({
                turn,
                contextTokens: lastInfo.tokens.input,
                type,
                activeT1: counts.t1,
                activeT2: counts.t2,
                t1Tokens: counts.t1Tok,
                t2Tokens: counts.t2Tok,
            })
        }
...
test("SIM 2: 25 T1 blocks pre-populated — T2 fires, T3 does not", async () => {
    ...
    try {
        ...
        const output = {
            messages: [makeUserMessage(ANCHOR, "Go"), makeAssistantMessage("a0", "OK", 45000)],
        }
        await handler({}, output)
        const suffix = getSuffixText(output)

        assert.ok(
            suffix.includes("[Tier 2 Trigger]"),
            `T2 should fire (12.5K > 10K). Got: ${suffix.substring(0, 200)}`,
        )
...
test("SIM 4: T2 cadence blocked → T3 fires (independent counters)", async () => {
    ...
    try {
        ...
        const output = {
            messages: [makeUserMessage(ANCHOR, "Go"), makeAssistantMessage("a0", "OK", 46000)],
        }
        await handler({}, output)
        const suffix = getSuffixText(output)

        assert.ok(
            suffix.includes("[Tier 3 Trigger]"),
            `T3 should fire (T2 cadence blocked, T3 open). Got: "${suffix.substring(0, 300)}"`,
        )
        assert.ok(!suffix.includes("[Tier 2 Trigger]"), "T2 should NOT fire (cadence blocked)")
    } finally {
        rmSync(tempDir, { recursive: true, force: true })
    }
})
...
test("SIM 5: empty state — no tier nudges fire", async () => {
    ...
    try {
        ...
        const output = {
            messages: [makeUserMessage(ANCHOR, "Hi"), makeAssistantMessage("a0", "Hello", 15000)],
        }
        await handler({}, output)
        const suffix = getSuffixText(output)
...
test("SIM 7: 30-turn session — no crashes, state consistent", async () => {
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            conversation.push(
                makeAssistantMessage(`seed-a${i}`, "y".repeat(2000), 15000 + i * 3000),
            )
        }
        ...
        for (let turn = 0; turn < 30; turn++) {
            ...
            if (suffix.includes("[Tier 2 Trigger]")) {
                ...
                for (const id of [...state.prune.messages.activeBlockIds]) {
                    const b = state.prune.messages.blocksById.get(id)
                    if (b?.active && (b.tier ?? 1) === 1) {
                        consumed.push(id)
                        t1Sum += b.summaryTokens
                    }
                }
...
test("SIM 8: T2 range excludes non-T1 blocks via cross-tier narrowing", async () => {
    ...
    try {
        ...
        assert.ok(
            suffix.includes("[Tier 2 Trigger]"),
            `T2 should fire. Got: ${suffix.substring(0, 200)}`,
        )
        ...
        for (let id = startId; id <= endId; id++) {
            ...
            if (b?.active) {
                assert.equal(
                    b.tier ?? 1,
                    1,
                    `Block ${id} in T2 range is tier ${b.tier}, should be 1`,
                )
            }
        }
    } finally {
