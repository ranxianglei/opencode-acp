...
export const injectCompressNudges = (
    ...
    // The global-percent default is deliberately LOW (5%): with the default
    // nudgeGrowthTokens (50K), a growth nudge's current tokens are always
    // >= baseline+50K, and a 5% floor only binds when 5% x window >
    // baseline+50K (i.e. windows >= ~2M for typical baselines). A 15% default
    // would bind on >=400K windows and shift every compress cycle's working
    // range upward (~2x average context on 1M-window models) — a silent,
    // bug-level behavior change for large-window users. Users who want a
    // higher floor set it (per-model or global) explicitly.
