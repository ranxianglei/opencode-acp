...
## Rebase onto master (post #233/#303)

- `lib/commands/decompress.ts` hunk dropped — file deleted by #233 (manualMode +
  redundant commands removal). The tool path fix in `lib/compress/decompress.ts`
  is the only remaining path; the Round 2 slash-command finding is moot.
- `lib/compress/status.ts`: conflict resolved by porting the three changes
  (all-blocks drilldown, `[inactive]` marker, active-only overview) onto
  master's refactored `buildStatusReport` (post #291 shape:
  `renderCompressedDrilldown` with `blocksById` + `getEffectiveCompressedTokens`,
  `renderOverview` with `systemTokens`).
- `tests/inactive-block-decompress.test.ts`: adapted to post-#233 tool factory
  API (`ToolFactoryContext` + `singletonRegistry` from `tests/registry-stub.ts`);
  removed deleted `SessionState` fields (`manualMode`, `pendingManualTrigger`),
  added required `qualityGateRetryPending` (post #303).
- Verified: `npm run typecheck` clean, `npm test` 1000/1000 pass.
