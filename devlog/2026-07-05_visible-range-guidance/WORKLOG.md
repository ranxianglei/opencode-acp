...
- **Entry point / key function**:
    ...
    - The same baseline auto-resets when `currentTokens` drops significantly below
      ...
      on the _post-compression_ level rather than waiting another 5% growth cycle.
...
- **Key logic explanation** (if non-trivial):
    ...
    - `clampMessageRef()`: when the model supplies an `endId` ref that is not
      registered in `state.messageIds.byRef` _and_ parses to a number higher than
      the last visible message's ref, the ref is clamped to the last visible
      message. Refs that _are_ registered but were consumed by a prior block still
...
- The "Block B nudge-text rewrite" originally deferred to a later iteration got
  ...
  no actionable breakdown of _where_ tokens were spent. The 3-category
  ...
  the current prompt is _actionable_; a prompt that says "you're at 47%" with no
...
- [ ] Watch for providers that omit `model.limit.context`; if more than a
      handful appear, consider a small _explicit_ fallback (user-configured
