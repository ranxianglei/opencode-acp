...
| 层级 | 频率 | 5 年内压缩次数 | 第 5 年摘要 token | 净上下文增长 |
|------|------|---------------|------------------|-------------|
| T1 | 约每 7 天 | ~255 次 | ~87K tok | — |
| T2 | 约每 10 个月 | ~5 次 | ~9K tok | 179 token/天 |
| T3 | 约每 8.5 年 | 0 次（未达到） | 0 tok | — |

第 5 年时，摘要总开销 ≈ **96K token**（占 1M 上下文的 9.6%）。压缩比自然控制频率：T1 摘要比原始内容慢 ~45× 增长，T2 摘要比 T1 慢 ~10×。上下文填满 1M 需要约 15 年（届时摘要约 150K token）— 对任何真实会话来说实际上是无限的。
