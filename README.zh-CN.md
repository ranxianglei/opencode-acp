...
**生命周期预测**（从空会话开始的 5 年模拟，1M 上下文窗口）：

| 层级 | 频率 | 5 年内压缩次数 | 第 5 年摘要 token |
|------|------|---------------|------------------|
| T1 | 约每 7 天 | ~255 | 39K（待处理，0–50K 波动） |
| T2 | 约每 12 个月 | ~5 | 25K（累积） |
| T3 | 约每 8.5 年 | 0 次（未达到） | 0 |

第 5 年时，摘要总开销 ≈ **64K token**，但可见上下文 ≈ **327K**（1M 的 32.7%）— 差异来自未压缩的原始内容（~37K，两次 T1 压缩之间波动）和系统开销（~22K）。净增长率：**179 token/天**。上下文填满 1M 需要约 15 年 — 对任何真实会话来说实际上是无限的。

**累计 token 节省**（计费指标 — 整个会话所有 API 调用发送的 token 总量）：
...
| 3 个月 | 133M | 14.8M | 89% |
| 6 个月 | 525M | 35.5M | 93% |
| 1 年 | 21.4 亿 | 9500 万 | 96% |
| 5 年 | 534 亿 | 14 亿 | 97% |

无 ACP 时，累计 token 消耗呈**二次增长** O(n²) — 上下文随每轮线性增长，每次 API 调用都重发全部上下文。有 ACP 时，上下文被压缩在有界范围内，累计消耗呈**线性增长** O(n)。节省比例随会话长度增加：1 个月节省 73%，1 年节省 96%。
...
当上下文达到 100% 时，系统自动截断老年代 block 摘要，防止上下文溢出。这是最后的兜底机制 — 有了三级压缩，GC 极少激活，因为 T2/T3 蒸馏将摘要开销控制在有界范围内（5 年预测中净增长 179 token/天）。
