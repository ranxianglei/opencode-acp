...
| prompt-cache 命中 token | 53.4 亿 |
| 平均 prompt-cache 命中率 | ~87% |
| 累计创建压缩 block | 4,894 个 |
| 压缩释放的 token | 6180 万 |

两个有代表性的重负载会话：

| 会话 | 跨度 | 回合数 | input token | 缓存读取 | 缓存命中 | 峰值上下文 |
|------|------|--------|-------------|----------|----------|------------|
| compute-problem | 6 天 | 2,694 | 8000 万 | 5.02 亿 | 86.2% | 48.8 万（1M 的 49%） |
| model-editing | 2 天 | 1,536 | 5090 万 | 4.12 亿 | 89.0% | 76.9 万（1M 的 77%） |

即便在 **77% 上下文占用**下，前缀缓存命中率仍接近 **90%**——这正是 ACP 缓存感知压缩的价值：它从尾部裁剪（保留共享前缀），而非盲目截断。
